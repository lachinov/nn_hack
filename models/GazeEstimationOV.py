...
class GazeEstimationOV(Model.BaseModel):
    def __init__(self,name, xml_path, bin_path, batch_size, image_key):
        ...
        self.FaceDetectionInputLayer = 'left_eye_image'
    ...
    def __call__(self, batch:dict) -> dict:
        ...
        l, r = image[:,0], image[:,1]
        headpose_angles = batch['headpose_estimation_headpose'].copy()
        headpose_angles[:,2] = 0


        results = self.FaceDetectionExecutable.infer(inputs={'left_eye_image': l,
                                                             'right_eye_image' : r,
                                                             'head_pose_angles': headpose_angles})

        batch[self.name+'_gaze'] = results['gaze_vector']
